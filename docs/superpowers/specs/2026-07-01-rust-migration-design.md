...
- **双工作模式**: 快捷粘贴模式（临时缓冲区）+ 文件编辑模式（打开并保存文件）
...
### 阶段 4: 文件编辑模式
- 命令行参数支持 (`snapvim file.txt`)
- 模式判断逻辑 (buffer 是否有文件名)
- 单实例处理 (IPC 通知已有实例)
- 系统文件关联注册 (安装时)
- 窗口标题显示文件名
...
SnapVim 支持两种工作模式：快捷粘贴模式和文件编辑模式。两种模式共享同一个 Neovim 实例，通过 buffer 是否有文件名来区分。

### 11.1 两种模式对比

| | 快捷粘贴模式 | 文件编辑模式 |
|---|---|---|
| 触发方式 | Ctrl+Space | `snapvim file.txt` / 文件关联打开 |
| 缓冲区 | 无名 scratch buffer | 关联具体文件路径 |
| `:w` 行为 | 复制到剪贴板 → 粘贴回原窗口 → 隐藏 | 保存到磁盘 |
| `:wq` 行为 | 同上 | 保存 → 隐藏窗口 |
| `:q` 行为 | 隐藏窗口 | 隐藏窗口（未保存提示） |
| 窗口标题 | "SnapVim" | "filename.txt - SnapVim" |

### 11.2 模式判断逻辑

Rust 层通过 `nvim_buf_get_name()` 判断当前 buffer 是否关联文件：
- 有文件名 → 文件编辑模式，`:w` 让 Neovim 正常保存（`nvim_command("write")`）
- 无文件名 → 快捷粘贴模式，`:w` 拦截为粘贴流程

不需要维护两套逻辑，Neovim 本身区分有文件名的 buffer 和无文件名的 buffer。
...
```
- 启动时传入文件路径
- spawn nvim 后执行 `:e file.txt`
- 窗口标题显示文件名

**2. 系统文件关联（优先）**
- 安装时注册常见文本文件类型关联
- OS 以命令行参数方式启动 SnapVim
- 与命令行参数走相同逻辑

**文件关联注册（安装时）：**
```
Windows (NSIS 安装器):
  注册 .txt, .md, .log, .json, .yaml, .yml, .toml, .ini, .cfg 等
  右键菜单: "Edit with SnapVim"
...
如果 SnapVim 已在运行，新打开的文件请求需要通知已有实例：
- 启动时检测是否已有实例运行（通过本地 socket/pipe 或 lock file）
- 已有实例 → 通过 IPC 通知打开新文件
- 已有实例打开新 buffer（切换或新 tab）
- 无实例 → 正常启动流程

### 11.5 拖拽文件到窗口（未来扩展）

- Tauri v2 支持拖拽事件
- 拖入文件 → `nvim_command(":e <filepath>")` → 从粘贴模式无缝切换到文件模式
- 复杂度低，可作为后续迭代快速添加
...
- **双工作模式**: 快捷粘贴模式（临时缓冲区，保存即粘贴回原窗口）+ 文件编辑模式（打开并保存文件）
- **核心优势**: 完整 Vim 功能 + 轻量安装包 (~5MB) + 跨平台
- **开发效率**: Web 前端 + 成熟生态
- **用户体验**: 保留全局热键、托盘常驻、保存即粘贴的核心体验，同时支持像普通 Vim 一样编辑文件

架构清晰、模块化、可扩展，为未来功能（LSP、插件系统、拖拽打开文件等）预留空间。
